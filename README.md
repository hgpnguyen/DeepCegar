GERAN <img width="100" alt="portfolio_view" align="right" src="http://safeai.ethz.ch/img/sri-logo.svg">
========

![High Level](https://raw.githubusercontent.com/eth-sri/eran/master/overview.png)

Guided ETH Robustness Analyzer for Neural Networks (GERAN) is a state-of-the-art sound, precise, and scalable analyzer based on [abstract interpretation](https://en.wikipedia.org/wiki/Abstract_interpretation) for the complete and incomplete robustness verification of MNIST, CIFAR10, and ACAS Xu based networks. GERAN produces state-of-the-art precision and performance for both complete and incomplete verification. GERAN is based on ERAN, which is developed at the [SRI Lab, Department of Computer Science, ETH Zurich](https://www.sri.inf.ethz.ch/) as part of the [Safe AI project](http://safeai.ethz.ch/). 

The goal of ERAN is to automatically verify robustness of neural networks with feedforward, convolutional, and residual layers against input perturbations (e.g.,  L∞-norm attacks, geometric transformations, etc). In comparison, GERAN is designed to overcome the imcompleteness issue of abstract interpretation technique. In specific, GERAN generalize adversary attack for neural networks into abstract domain attack for partial neural networks, which we named as ABA(abstract domain attack). GERAN integrete ABA with the classical abstract interpretation, and attempt to witness the improper overapproximation in advance, and thus gives a chance for abstract interpretation engine to tune the precision as early as possible. Meanwhile, this also makes GERAN much easier to refine the abstract domain, since it can pinpoint the refine site much more precise, and thus the propagating chain is very short. For ERAN, its propagating chain is long, which not only spreads over the whole network, and pinpointing the refine site is almost impossible.

Same as ERAN, GERAN supports networks with ReLU, Sigmoid and Tanh activations and is sound under floating point arithmetic. It employs custom abstract domains which are specifically designed for the setting of neural networks and which aim to balance scalability and precision. Specifically, ERAN supports the following three analysis:
...
All analysis are implemented using the [ELINA](http://elina.ethz.ch/) library for numerical abstractions. We also improve ELINA to support powerset domain which is needed for domain refinements. More details can be found in the publications below. 


GERAN decompose a given network into operations using tensorflow APIs. Based on those operations, GERAN constructs concrete model, deepzono model, or deeppoly model on the fly. All these models are IR-based, and consists of a list of IRs. Their positions are identical, and thus we may croess refer specific position of the network in these models. 

Currently, we focus on developing on and testing zonotope domain. So please use `--domain deepzono` as the argument when you seek to test our system. 


Command line options
--------------------
We add some command line options: 
* ```<use_abstract_attack>```: specifies whether abstract domain attack is enabled.
* ```<use_abstract_refine>```: specifies whether abstract domain attack is enabled.
* ```<attack_method>```: specifies which attack method will be adapted, such as `scipy` or `pgd`.
* ```<domain>```: please use 'deepzono' for now.


ELINA version: 
---------------------
We modified the original elina, as elina is still in active development, we decide not to follow up their progress.

The detached commit is #856bcf, date: Feb 7, 2020

We removed fppoly_gpu support as we do not support gpu for now.

Please use our elina repo within this project.
...
--------------------
--------------------
Backup, Ignore please.
--------------------
The following is copied from ERAN, which needs to be updated later.
