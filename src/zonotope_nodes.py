...
class DeepzonoMatmul:
    ...
    def transformer(self, nn, man, element):
        """
...
class DeepzonoAffine(DeepzonoMatmul):
    ...
    def transformer(self, nn, man, element):
        ...
        """
        offset, old_length = self.abstract_information
        # get_arguments ==> man, True, element, offset+old_length, matrix_xpp, new_length, offset, old_length
...
class DeepzonoConv:
    ...
    def get_arguments(self, nn, man, element):
        """
        used to get the arguments to the transformer, also used by the child class
        Note: this function also adds the necessary dimensions, removing the old ones after the transformer is the responsibility of the caller
...
class DeepzonoRelu(DeepzonoNonlinearity):    
    ...
    def transformer(self, nn, man, element):
        ...
        """
        offset, length = self.abstract_information
        element = relu_zono_layerwise(*self.get_arguments(man, element), True)
        return element
...
class DeepzonoPool:
    ...
    def transformer(self, nn, man, element):
        ...
        element = pool_zono(man, True, element, (c_size_t * 3)(h,w,1), (c_size_t * 3)(H, W, C), 0, (c_size_t * 2)(self.stride[0], self.stride[1]), 3, offset+old_length, self.pad_top, self.pad_left, self.output_shape)
        element = remove_dimensions(man, element, offset, old_length)
        return element
