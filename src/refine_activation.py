...
def refine_activation_with_solver_bounds(nn, self, man, element, nlb, nub, relu_groups, timeout_lp, timeout_milp, use_default_heuristic, domain, use_krelu=True):
    """
    ...
    #print("Layer num", layerno)
    #print("predecessor", nn.predecessors)
    ...
    if nn.activation_counter==0:
        ...
        if config.refine_neurons==True:
            resl, resu, indices = get_bounds_for_layer_with_milp(nn, nn.in_LB, nn.in_UB, predecessor_index, predecessor_index, length, nlb, nub, relu_groups, use_milp,  candidate_vars, timeout)
