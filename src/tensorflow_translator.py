...
class TFTranslator:
    ...
    def __init__(self, model, session = None):
        """
        This constructor takes a reference to a TensorFlow Operation or Tensor or Keras model and then applies the 
        two TensorFlow functions graph_util.convert_variables_to_constants and graph_util.remove_training_nodes to 
        cleanse the graph of any nodes that are linked to training. This leaves us with the nodes you need for inference. 
        In the resulting graph there should only be tf.Operations left that have one of the following types 
        [Const, MatMul, Add, BiasAdd, Conv2D, Reshape, MaxPool, Placeholder, Relu, Sigmoid, Tanh]
        ...
        model : tensorflow.Tensor or tensorflow.Operation or tensorflow.python.keras.engine.sequential.Sequential or 
            ...
            if tensorflow.Tensor: model.op will be treated as the output node of the TensorFlow model. 
                Make sure that the graph only contains supported operations after applying
                graph_util.convert_variables_to_constants and graph_util.remove_training_nodes with [model.op.name] 
                as output_node_names
            if tensorflow.Operation: model will be treated as the output of the TensorFlow model. 
                Make sure that the graph only contains supported operations after applying 
                graph_util.convert_variables_to_constants and graph_util.remove_training_nodes with [model.op.name] 
                as output_node_names
            if tensorflow.python.keras.engine.sequential.Sequential: x = model.layers[-1].output.op.inputs[0].op 
                will be treated as the output node of the Keras model. Make sure that the graph only contains supported 
                operations after applying graph_util.convert_variables_to_constants and graph_util.remove_training_nodes 
                with [x.name] as output_node_names
            if keras.engine.sequential.Sequential: x = model.layers[-1].output.op.inputs[0].op will be treated as the 
                output node of the Keras model. Make sure that the graph only contains supported operations after applying 
                graph_util.convert_variables_to_constants and graph_util.remove_training_nodes with [x.name] as output_node_names
        session : tf.Session
            session which contains the information about the trained variables. If None the code will take the Session 
            from tf.get_default_session(). If you pass a keras model you don't have to
        ...
        # print ('as graph def: ', model.graph)
    ...
    def collect_resources(self, resources, res, deepbox_res=None, deepzono_res=None, deeppoly_res=None):
        if deepbox_res is None:
            deepbox_res=res
        if deepzono_res is None:
            deepzono_res=res
        if deeppoly_res is None:
            deeppoly_res=res
        res = {'concrete':res, 'deepbox':deepbox_res, 'deepzono':deepzono_res, 'deeppoly':deeppoly_res}
    ...
    def translate(self):
        ...
        translate() takes that graph_def, imports it, and 
            translates it into two lists which then can be processed by an Optimzer object.
        ...
        with tf.Graph().as_default() as graph:
            with tf.Session() as sess:
                ...
                for op in graph.get_operations(): 
                    ...
                    #     print ('      |==> input  (', i, ')  ', inp, sep='')
                    # for i, outp in enumerate(op.outputs):
                    #     print ('      |<== output (', i, ')  ', outp, sep='')
                    if op.type in operations_to_be_ignored_without_reshape:
                        # print ('   ------ being ignored without reshape ------', sep='')
                        continue
                    elif op.type in operations_to_be_ignored:
                        # print ('   ------ being ignored with reshape ------', sep='')
                    ...
                    for inp in op.inputs:
                        name = inp.name
                        kind = inp.op.type
                        shape = tensorshape_to_intlist(inp.shape)
                        if kind in operations_to_be_ignored: # input was reshaped
                    ...
                    in_out_info = (input_tensor_names, input_tensor_shapes, op.outputs[0].name, tensorshape_to_intlist(op.outputs[0].shape))
                    # print('in_out_info: ', in_out_info)


                    if op.type == "MatMul":
                        deeppoly_res = self.matmul_resources(op) + in_out_info
                        deepzono_res = deeppoly_res
                        # self.collect_resources(operation_resources, res)
                        # operation_resources.append(res)
                    elif op.type == "Add":
                        ...
                        if left_type == 'Const' and right_type == 'Const':
                            ...
                            # self.collect_resources(operation_resources, in_out_info)
                            # res = in_out_info
                            # operation_resources.append(res)
                        ...
                        deepzono_res = deeppoly_res
                        # res = in_out_info
                        # print('nonlinear: peek resources size:', len(res))
                        # print(res)
                        # self.collect_resources(operation_resources, res)
                        # operation_resources.append(res)
    ...
    def matmul_resources(self, op):
        """
        checks which one of the direct ancestor tf.Operations is a constant and returns the underlying tensor 
        as a numpy.ndarray inside a tuple. The matrix is manipulated in a way that it can be used as the 
        left multiplier in the matrix multiplication.
