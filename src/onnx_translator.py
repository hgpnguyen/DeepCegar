...
class ONNXTranslator:
	...
	def collect_resources(self, resources, res, deepzono_res=None, deeppoly_res=None):
		if deepzono_res is None: 
			deepzono_res=res
		if deeppoly_res is None:
			deeppoly_res=res 
		res = {'concrete':res, 'deepzono':deepzono_res, 'deeppoly':deeppoly_res} 
	...
	def translate(self):
		...
		self.collect_resources(operation_resources, in_out_placeholder) 
		reshape_map = {}
		...
		for node in self.nodes:
			...
			if node.op_type == "MatMul":
				res = self.matmul_resources(node) + in_out_info 
				self.collect_resources(operation_resources, res)
			elif node.op_type == "Gemm":
				res = self.gemm_resources(node) + in_out_info 
				self.collect_resources(operation_resources, res)
				...
				if left_type == 'Constant' and right_type == 'Constant':
					...
					res = self.add_resources(node) + in_out_info 
					self.collect_resources(operation_resources, res)
				else:
					if node.op_type != "Add":
				...
				if left_type == 'Constant' and right_type == 'Constant':
					...
					res = self.sub_resources(node) + in_out_info 
					self.collect_resources(operation_resources, res)
				else:
				...
				res = (filters, bias, image_shape, strides, pad_top, pad_left) + in_out_info
				self.collect_resources(operation_resources, res)
			elif node.op_type == "MaxPool":
				image_shape, kernel_shape, strides, padding, dilations, pads, ceil_mode, storage_order = self.maxpool_resources(node)
				res =  (image_shape, kernel_shape, in_out_info[2]) + in_out_info
				...
				self.collect_resources(operation_resources, res, deepzono_res, deeppoly_res)
			elif node.op_type == "Placeholder":
				assert 0, "Placeholder is not in the ONNX graph"
				res = in_out_info 
				self.collect_resources(operation_resources, res)
			elif node.op_type in ["Relu", "Sigmoid", "Tanh"]:
				res = self.nonlinearity_resources(node) + in_out_info 
				self.collect_resources(operation_resources, res)
			...
			# Gather is for the moment solely for shapes
				...
				if only_shape:
					...
					res = (image_shape, indexes, axis) + in_out_info 
					self.collect_resources(operation_resources, res)
